function VoteFallbackPanel({ side }: { side: "home" | "away" }) {
  ...
  return (
    <article className="relative flex flex-col items-center justify-center gap-2 overflow-hidden border-0 border-broadcast-dim/25 px-2.5 pt-4 pb-3 sm:items-stretch sm:justify-start sm:gap-3 sm:rounded-[10px] sm:border sm:bg-white/[0.04] sm:p-4 sm:pt-5">
      <span
        aria-hidden="true"
        className={
          isHome
      ...
      />
      <div className="h-2.5 w-20 rounded-sm bg-white/10 sm:h-3 sm:w-28" />
      <div className="h-6 w-32 rounded-sm bg-white/10 sm:h-10 sm:w-56" />
      ...
      <div className="h-2.5 w-24 rounded-sm bg-white/10 sm:hidden" />
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative grid min-h-[440px] grid-cols-2 sm:min-h-0 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] sm:gap-3">
        ...
        <div
          aria-hidden="true"
          className="absolute top-[44%] left-1/2 z-10 grid h-[42px] w-[42px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-broadcast-gold/60 bg-broadcast-night sm:hidden"
        >
          <div className="h-4 w-7 rounded-sm bg-broadcast-gold/30" />
        </div>
        <div className="hidden sm:flex sm:items-center sm:justify-center sm:px-5">
          <div className="h-9 w-14 rounded-sm bg-broadcast-gold/30" />
        </div>
        <VoteFallbackPanel side="away" />
      </div>
      <div className="h-4 w-36 self-center rounded-sm bg-white/[0.07]" />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <article className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-x-4 gap-y-3 rounded-[10px] border border-broadcast-gold/35 bg-broadcast-gold/[0.06] p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
        <div className="h-16 w-16 rounded-sm bg-white/10 sm:h-20 sm:w-20" />
        <div>
          <div className="h-3 w-28 rounded-sm bg-white/10" />
          <div className="mt-2 h-9 w-52 rounded-sm bg-white/10" />
        </div>
        <div className="col-span-2 flex gap-6 sm:col-span-1">
          {[0, 1, 2].map((index) => (
            <div key={index}>
              <div className="h-2.5 w-10 rounded-sm bg-white/10" />
              <div className="mt-1.5 h-6 w-14 rounded-sm bg-white/10" />
            </div>
          ))}
        </div>
      </article>
      ...
      <div className="grid gap-1.5">
        {Array.from({ length: 8 }, (_, row) => (
          ...
          >
            <div className="h-3 w-5 rounded-sm bg-white/10" />
            <div className="h-11 w-11 rounded-sm bg-white/10" />
            <div className="h-6 w-32 rounded-sm bg-white/10" />
            <div className="h-4 w-10 rounded-sm bg-white/10" />
            <div className="hidden h-4 w-10 rounded-sm bg-white/10 sm:block" />
            <div className="hidden h-4 w-10 rounded-sm bg-white/10 sm:block" />
            <div className="hidden h-4 w-10 rounded-sm bg-white/10 sm:block" />
          </div>
        ))}
